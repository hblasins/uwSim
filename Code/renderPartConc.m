...
% This script renders 4 images with different scattering modes.
% The modes are as follows:

% 1. default = include all scattering from 0 to 180 degrees
% 2. forward =  only include scattering from 0 and 90 to 180 degrees
% 3. backward = only include scattering from 0 to 90 degrres
% 4. direct = only include scattering at 0 degree
...
smallParticleConc = [0 0.01 0.1 0.3 0.5];
largeParticleConc = [0 0.01 0.1 0.3 0.5];

absorptionFiles = cell(nConditions,1);
scatteringFiles = cell(nConditions,1);
phaseFiles = cell(nConditions,1);

assert(nConditions == length(largeParticleConc) && nConditions == length(smallParticleConc));
